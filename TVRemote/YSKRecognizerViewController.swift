...
class YSKRecognizerViewController: UITableViewController, YSKRecognizerDelegate {
    ...
    func onExitButtonTap() {
        let vc = self.storyboard?.instantiateViewControllerWithIdentifier("browserview") as! ViewController?
        if (vc != nil) {
    ...
    override func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        let cell = tableView.cellForRowAtIndexPath(indexPath) as? YSKRecognizerCell
        let query = cell?.resultLabel?.text
        if (query != nil) {
            let vc = self.storyboard?.instantiateViewControllerWithIdentifier("browserview") as! ViewController?
            if (vc != nil) {
                self.presentViewController(vc!, animated: true, completion: nil)
                vc!.loadSearchEngine(query)
            }
